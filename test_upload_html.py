#!/usr/bin/env python3
"""
Тест загрузки HTML файлов через API
"""
import time
import requests
import sys
...
def find_html_files() -> List[Path]:
    """Найти все HTML файлы в текущей директории"""
    html_files = []
...
def check_server() -> bool:
    ...
    try:
        ...
        print("   Убедитесь, что сервер запущен: docker-compose up -d api")
        return False
...
def upload_single_file(file_path: Path) -> Dict[str, Any]:
    """Загрузка одного HTML файла через API"""
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            response = requests.post(
                ...
                files=files,
                timeout=120  # HTML может обрабатываться дольше
            )
...
def check_task_status(task_id: str, filename: str, max_wait: int = 120) -> Dict[str, Any]:
...
def main():
    ...
    print("\n" + "="*70)
    print("ТЕСТ ЗАГРУЗКИ HTML ФАЙЛОВ ЧЕРЕЗ API")
    print("="*70)
    ...
    for i, html_file in enumerate(html_files, 1):
        ...
        print(f"   {i}. {html_file.name} ({size:.1f} KB)")
    
    # Загружаем каждый файл
    results = []
    for i, html_file in enumerate(html_files, 1):
        print(f"\n{'='*70}")
        print(f"Файл {i}/{len(html_files)}: {html_file.name}")
        print('='*70)
        
        # Загрузка файла
        upload_result = upload_single_file(html_file)
        ...
        if upload_result.get('success'):
            ...
            if task_id:
                ...
                status_result = check_task_status(task_id, html_file.name)
    ...
    if all_success:
        ...
        print(f"   Проверьте логи Celery: docker-compose logs -f celery_worker")
